...
abstract class BaseOwner extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    const PEER = 'CompanyBundle\\Model\\OwnerPeer';
     ...
     * @return Owner The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[] = OwnerPeer::ID;
        }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[] = OwnerPeer::USER_ID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 2; // 2 = OwnerPeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating Owner object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = OwnerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                OwnerPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(OwnerPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(OwnerPeer::USER_ID)) {
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = OwnerPeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = OwnerPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Owner'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Owner'][$this->getPrimaryKey()] = true;
        $keys = OwnerPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OwnerPeer::ID)) $criteria->add(OwnerPeer::ID, $this->id);
        if ($this->isColumnModified(OwnerPeer::USER_ID)) $criteria->add(OwnerPeer::USER_ID, $this->user_id);
    ...
    {
        $criteria = new Criteria(OwnerPeer::DATABASE_NAME);
        $criteria->add(OwnerPeer::ID, $this->id);
     ...
     * If this Owner is new, it will return
     ...
     * @return PropelObjectCollection|CompanyOwner[] List of CompanyOwner objects
     */
    public function getCompanyOwnersJoinCompany($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = CompanyOwnerQuery::create(null, $criteria);
    ...
    protected function doAddCompany(Company $company)
    {
        ...
        if (!$company->getOwners()->contains($this)) { $companyOwner = new CompanyOwner();
            $companyOwner->setCompany($company);
            $this->addCompanyOwner($companyOwner);

            $foreignCollection = $company->getOwners();
    ...
    {
        return (string) $this->exportTo(OwnerPeer::DEFAULT_STRING_FORMAT);
    }
