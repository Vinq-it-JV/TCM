...
namespace UserBundle\Model\Base;
...
use UserBundle\Model\User as ChildUser;
use UserBundle\Model\UserQuery as ChildUserQuery;
use UserBundle\Model\UserRole as ChildUserRole;
use UserBundle\Model\UserRoleQuery as ChildUserRoleQuery;
use UserBundle\Model\Map\UserRoleTableMap;
 ...
 *
 *
 * @package    propel.generator.src.UserBundle.Model.Base
 */
abstract class User implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\UserBundle\\Model\\Map\\UserTableMap';
    ...
    /**
     * Initializes internal state of UserBundle\Model\Base\User object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\UserBundle\Model\User The current object (for fluent API support)
     */
    ...
    public function setLogins($v)
    {
        ...
        if ($this->logins !== $v) {
            $this->logins = $v;
            $this->modifiedColumns[UserTableMap::COL_LOGINS] = true;
        }
    ...
    public function setLanguage($v)
    {
        ...
        if ($this->language !== $v) {
            $this->language = $v;
            $this->modifiedColumns[UserTableMap::COL_LANGUAGE] = true;
        }
    ...
    public function setIsEnabled($v)
    {
        ...
        if ($this->is_enabled !== $v) {
            $this->is_enabled = $v;
            $this->modifiedColumns[UserTableMap::COL_IS_ENABLED] = true;
        }
    ...
    public function setIsExternal($v)
    {
        ...
        if ($this->is_external !== $v) {
            $this->is_external = $v;
            $this->modifiedColumns[UserTableMap::COL_IS_EXTERNAL] = true;
        }
    ...
    public function setIsLocked($v)
    {
        ...
        if ($this->is_locked !== $v) {
            $this->is_locked = $v;
            $this->modifiedColumns[UserTableMap::COL_IS_LOCKED] = true;
        }
    ...
    public function setIsExpired($v)
    {
        ...
        if ($this->is_expired !== $v) {
            $this->is_expired = $v;
            $this->modifiedColumns[UserTableMap::COL_IS_EXPIRED] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : UserTableMap::translateFieldName('Logins', TableMap::TYPE_PHPNAME, $indexType)];
            $this->logins = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : UserTableMap::translateFieldName('Language', TableMap::TYPE_PHPNAME, $indexType)];
            $this->language = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : UserTableMap::translateFieldName('IsEnabled', TableMap::TYPE_PHPNAME, $indexType)];
            $this->is_enabled = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 10 + $startcol : UserTableMap::translateFieldName('IsExternal', TableMap::TYPE_PHPNAME, $indexType)];
            $this->is_external = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 11 + $startcol : UserTableMap::translateFieldName('IsLocked', TableMap::TYPE_PHPNAME, $indexType)];
            $this->is_locked = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 12 + $startcol : UserTableMap::translateFieldName('IsExpired', TableMap::TYPE_PHPNAME, $indexType)];
            $this->is_expired = (null !== $col) ? (boolean) $col : null;
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\UserBundle\\Model\\User'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->userRolesScheduledForDeletion !== null) {
                if (!$this->userRolesScheduledForDeletion->isEmpty()) {
                    \UserBundle\Model\UserRoleQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGINS)) {
            $modifiedColumns[':p' . $index++]  = 'logins';
        }
        if ($this->isColumnModified(UserTableMap::COL_LANGUAGE)) {
            $modifiedColumns[':p' . $index++]  = 'language';
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_ENABLED)) {
            $modifiedColumns[':p' . $index++]  = 'is_enabled';
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_EXTERNAL)) {
            $modifiedColumns[':p' . $index++]  = 'is_external';
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_LOCKED)) {
            $modifiedColumns[':p' . $index++]  = 'is_locked';
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_EXPIRED)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\UserBundle\Model\User
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[7], $arr)) {
            $this->setLogins($arr[$keys[7]]);
        }
        if (array_key_exists($keys[8], $arr)) {
            $this->setLanguage($arr[$keys[8]]);
        }
        if (array_key_exists($keys[9], $arr)) {
            $this->setIsEnabled($arr[$keys[9]]);
        }
        if (array_key_exists($keys[10], $arr)) {
            $this->setIsExternal($arr[$keys[10]]);
        }
        if (array_key_exists($keys[11], $arr)) {
            $this->setIsLocked($arr[$keys[11]]);
        }
        if (array_key_exists($keys[12], $arr)) {
            $this->setIsExpired($arr[$keys[12]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\UserBundle\Model\User The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGINS)) {
            $criteria->add(UserTableMap::COL_LOGINS, $this->logins);
        }
        if ($this->isColumnModified(UserTableMap::COL_LANGUAGE)) {
            $criteria->add(UserTableMap::COL_LANGUAGE, $this->language);
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_ENABLED)) {
            $criteria->add(UserTableMap::COL_IS_ENABLED, $this->is_enabled);
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_EXTERNAL)) {
            $criteria->add(UserTableMap::COL_IS_EXTERNAL, $this->is_external);
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_LOCKED)) {
            $criteria->add(UserTableMap::COL_IS_LOCKED, $this->is_locked);
        }
        if ($this->isColumnModified(UserTableMap::COL_IS_EXPIRED)) {
            $criteria->add(UserTableMap::COL_IS_EXPIRED, $this->is_expired);
        }
        if ($this->isColumnModified(UserTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \UserBundle\Model\User (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \UserBundle\Model\User Clone of current object.
     * @throws PropelException
     */
    ...
    public function initUserRoles($overrideExisting = true)
    {
        ...
        $this->collUserRoles->setModel('\UserBundle\Model\UserRole');
    }
